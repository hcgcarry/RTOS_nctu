    ...
    if (OSRunning == TRUE) {
        ...
        if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Reschedule only if all ISRs complete ... */
            ...
            if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy */
                ...
                //tmpOSPrioCur = (int)OSPrioCur;
                //tmpOSPrioHighRdy = (int)OSPrioHighRdy;
                /*
                tmpOSPrioCur |= OSPrioCur;
                tmpOSPrioHighRdy |= OSPrioHighRdy;
                sprintf(&CtxSwMessage[CtxSwMessageTop++],"%5d preempty %d   %d\n",(int)OSTime,tmpOSPrioCur,tmpOSPrioHighRdy);
                */
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == TRUE) {    
        ...
        while (ptcb->OSTCBPrio != OS_IDLE_PRIO) {          /* Go through all TCBs in TCB list          */
            OS_ENTER_CRITICAL();
            if (ptcb->OSTCBDly != 0) {                     /* note: waiting for event ,set OSTCBDly to 0:Delayed or waiting for event with TO     */
