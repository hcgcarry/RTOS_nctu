...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    if (OSTCBCur->OSTCBPrio == cpp) {                 /* Did we have to raise current task's priority? */
        ...
        OSPrioCur = OSTCBCur->OSTCBPrio;
        // sprintf(&CtxSwMessage[CtxSwMessageTop++],"after %5d unlock R%d   (Prio=%d changes to=%d)\n",(int)OSTime,(int)cpp,(int)cpp,(int)OSTCBCur->OSTCBPrio,(int)OSPrioCur);
        // sprintf(&CtxSwMessage[CtxSwMessageTop++],"after %5d unlock R%d   OSPrioCur %d\n",(int)OSTime,(int)cpp,(int)cpp,(int)OSPrioCur);
    }
    else{
